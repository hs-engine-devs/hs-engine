...
class RainShaderGLSL extends FlxShader
{
    ...
    public function updateViewInfo(screenWidth:Float, screenHeight:Float, camera:FlxCamera):Void {
        ...
        var viewLeft = camera.scroll.x;
        var viewTop = camera.scroll.y;
        var viewRight = camera.scroll.x + camera.width;
        var viewBottom = camera.scroll.y + camera.height;

        uCameraBounds.value = [viewLeft, viewTop, viewRight, viewBottom];
    }
}
